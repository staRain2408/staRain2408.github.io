...
function draw_bar(t, f, n){
    const ctx = document.getElementById('pitcher_num_Chart').getContext('2d');
...
function draw_bar_percent(t, f, n){
    const ctx = document.getElementById('pitcher_percent_Chart').getContext('2d');
...
function draw_PR(f, n){
    const ctx = document.getElementById('pitcher_PR_Chart').getContext('2d');

        var RadarChart = new Chart(ctx, {
            type: 'radar',
            data: {
                labels: ['三振率', '三振保送比', '防禦率', '被安打率', '保送率', '每局被上壘率'], 
                datasets: [{
                    label: f + '投球成績(PR值)',
                    data: n,
                    fill: true,
